                                ...
                                for (int i = 0; i < sourceNodes.getLength(); i++) {
                                    ...
                                    if (rule.getField() != null && rule.getField().isCoalesce()) {
                                        if (rule.getField().getIds() != null) {
                                            ...
                                            for (int j = 0; j < trustedNodes.getLength(); j++) {
                                                String key = buildKeyForNode(trustedNodes.item(j), rule.getField().getIds());
                                                LOG.fine("found node with key: " + key);
                                                coalesceMatches.put(key, trustedNodes.item(j));
                                            }

                                            String newKey = buildKeyForNode(sourceNode, rule.getField().getIds());
                                    ...
                                    if (matchingTrustedNode == null) {
                                        if (!ignore) {
                                            ...
                                            trustedParentNode.appendChild(newNode);
                                        }
                                    } else {
                                        if (rule.getField() == null || !rule.getField().isStop()) {
